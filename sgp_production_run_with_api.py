...
UPDATED VERSION with real player stats integration.

Usage:
  With BOTH API keys (recommended for real stats):
    python3 sgp_production_run_with_api.py YOUR_ODDS_API_KEY YOUR_BALLDONTLIE_API_KEY

  With just Odds API (uses demo player stats):
    python3 sgp_production_run_with_api.py YOUR_ODDS_API_KEY

Changes from original sgp_production_run.py:
1. Added BallDontLie API integration for real player stats
2. Automatic injury status detection
3. Current roster verification
4. More accurate projections based on actual 2024-25 season data
"""
...
from data.balldontlie_api import BallDontLieAPI  # NEW: Import BallDontLie API
...
def main():
    """Main SGP production run with BallDontLie API integration"""

    if len(sys.argv) < 2:
        print("Usage:")
        print("  With BallDontLie API (recommended):")
        print("    python3 sgp_production_run_with_api.py YOUR_ODDS_API_KEY YOUR_BALLDONTLIE_API_KEY")
        print()
        print("  Without BallDontLie API (uses demo stats):")
        print("    python3 sgp_production_run_with_api.py YOUR_ODDS_API_KEY")
        print()
        print("Get BallDontLie API key at: https://www.balldontlie.io")
        sys.exit(1)
    ...
    # NEW: Initialize BallDontLie API if key provided
    ...
    if balldontlie_api_key:
        ...
        print("✓ API connected - will use real 2024-25 season stats")
    else:
        print("ℹ No BallDontLie API key provided - using demo stats")
        print("  (Provide BallDontLie API key as 2nd argument for real data)")

    print()

    # Initialize components
    print("Initializing analysis components...")
    props_api = PlayerPropsAPI(odds_api_key)
    stats_model = PlayerStatsModel(balldontlie_api=balldontlie_api)  # UPDATED: Pass API
    ...
    for prop in all_props[:50]:  # Limit to first 50 for performance
        # NEW: Auto-fetch injury status if API available
        injury_status = None
        if balldontlie_api:
            injury_status = stats_model.get_player_injury_status(prop.player_name)
            if injury_status and injury_status != 'healthy':
                print(f"⚠ {prop.player_name} - Injury Status: {injury_status.upper()}")
        ...
        projection = stats_model.project_player_prop(
            ...
            injury_status=injury_status  # UPDATED: Use real injury status
        )
    ...
    print("=" * 100)
    print()
    print("1. ALWAYS VERIFY INJURY STATUS")
    print("   - Check latest reports 90 minutes before tipoff")
    print("   - Late scratches can invalidate analysis")
    print()
    print("2. REAL STATS ARE MORE ACCURATE")
    if balldontlie_api:
        print("   ✓ Using real 2024-25 season stats from BallDontLie API")
    else:
        print("   ⚠ Using demo stats - get BallDontLie API key for real data")
        print("   Sign up at: https://www.balldontlie.io")
    print()
    print("3. TRACK YOUR RESULTS")
    print("   - Need 50+ bets to validate edge")
    print("   - Monitor calibration over time")
    print()
    print("Good luck! 🍀")
    print()
