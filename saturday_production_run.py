#!/usr/bin/env python3
"""
Saturday Production Run - January 17, 2026
Fetch all NBA games with start times, then run full analysis
"""
...
def analyze_game(game, model, efficiency_analyzer, variance_analyzer):
    ...
    if h2h_market:
        for outcome in h2h_market['outcomes']:
            ...
            if len(h2h_market['outcomes']) == 2:
                home_ml = next((o['price'] for o in h2h_market['outcomes'] if o['name'] == home_team), -110)
                away_ml = next((o['price'] for o in h2h_market['outcomes'] if o['name'] == away_team), -110)
    ...
    if spreads_market:
        for outcome in spreads_market['outcomes']:
            ...
            # If home team spread is -7.5, they need to win by 8+
            # Use normal distribution around projected margin
    ...
    if totals_market:
        for outcome in totals_market['outcomes']:
            ...
            # Simple model: use league average ~220 points, adjust by team strength
...
def main():
    ...
    if len(sys.argv) < 2:
        print("=" * 100)
        print("🏀 SATURDAY PRODUCTION RUN - NBA BETTING SIMULATOR")
        print("=" * 100)
        print()
        print("Usage: python3 saturday_production_run.py YOUR_API_KEY")
        print()
    ...
    # Display schedule
    display_games_schedule(games)

    # Ask user if they want to continue with analysis
    print()
    response = input("Continue with full betting analysis? (y/n): ").strip().lower()

    if response != 'y':
        print("Exiting. Schedule displayed above.")
        sys.exit(0)
    ...
    for i, game in enumerate(games, 1):
        ...
        game_opps = analyze_game(game, model, efficiency_analyzer, variance_analyzer)
    ...
    if not positive_ev_bets:
        ...
        for i, opp in enumerate(positive_ev_bets[:10], 1):  # Top 10
            line_str = f" {opp['line']:+.1f}" if opp['bet_type'] in ['spread'] else ""
            if opp['bet_type'].startswith('total_'):
    ...
    print("3. Sample size matters - need 100+ bets to validate edge")
    print("4. Never bet more than you can afford to lose")
    print("5. Use proper bankroll management (1/4 Kelly or smaller)")
    print()
